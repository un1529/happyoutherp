(function createErpBackend() {
  ...
  const backend = {
    ...
    async loadProfile() {
      ...
      const { data, error } = await client.from("profiles").select("id, display_name, role, job_title").eq("id", userId).maybeSingle();
      if (!error) { backend.profile = data; return data; }
      const fallback = await client.from("profiles").select("id, display_name, role").eq("id", userId).maybeSingle();
    ...
    async loadSharedState() {
      ...
      const { data, error } = await client.from("erp_state").select("data").eq("id", "main").maybeSingle();
      if (error) throw error;
    ...
    async saveSharedState(data) {
      ...
      const payload = { id: "main", data, updated_at: new Date().toISOString(), updated_by: backend.session.user.id };
      const { error } = await client.from("erp_state").upsert(payload);
      if (error) throw error;
    },

    canEdit() { return backend.profile?.role === "accountant"; },
  };
